...
def rec_prob_new(tensor, num_filters, Height, Width, c_state, h_state, k=3, act=tf.tanh):
  ...
  with tf.variable_scope("CNN_output_1"):
      y2 = tf.expand_dims(y2, axis=1)
      y3 = functions.recurrent_cnn(y2, 1, layer=4, num_filters=num_filters, stride=1,
                                   out_filters=num_filters, kernel=[k, k], act=tf.nn.relu, act_last=None)
      y3 = tf.squeeze(y3, axis=1)

  with tf.variable_scope("CNN_output_2"):

      y4 = functions.recurrent_cnn(y2, 1, layer=4, num_filters=num_filters, stride=1,
                                   out_filters=num_filters, kernel=[k, k], act=tf.nn.relu, act_last=None)
      y4 = tf.squeeze(y4, axis=1)

  y5 = tf.concat([y3, y4], axis=-1)

  return y5, c_state_out, h_state_out
